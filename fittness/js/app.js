(() => {
    ...
    function menuInit(timeout = 400) {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".icon-menu")) {
                bodyLockToggle(timeout);
                document.documentElement.classList.toggle("menu-open");
            } else if (bodyLockStatus && !e.target.closest(".panel-menu") && document.documentElement.classList.contains("menu-open")) {
                bodyUnlock(timeout);
    ...
    let formValidate = {
        ...
        addError(formRequiredItem) {
            ...
            const successMessage = formRequiredItem.parentElement.querySelector(".form__success");
            const timeoutShowErrorMessage = successMessage || inputError ? 300 : 0;
            const buttonSubmit = formRequiredItem.parentElement.querySelector('button[type="submit"]');
            ...
            if (inputError) {
                inputError.classList.remove("_show");
                setTimeout((() => {
                    inputError.remove();
                    buttonSubmit.removeAttribute("disabled");
                }), timeoutShowErrorMessage);
            }
            if (formRequiredItem.dataset.error) {
                const errorMessage = document.createElement("div");
                errorMessage.className = "form__error";
                errorMessage.textContent = formRequiredItem.dataset.error;
                formRequiredItem.parentElement.append(errorMessage);
                setTimeout((() => {
        ...
        removeError(formRequiredItem) {
            ...
            const formError = formRequiredItem.parentElement.querySelector(".form__error");
            if (formError) {
                formError.classList.remove("_show");
                setTimeout((() => {
                    formRequiredItem.parentElement.removeChild(formError);
                }), 300);
            }
        },
        addSuccess(formRequiredItem) {
            if (formRequiredItem.parentElement.querySelector(".form__success")) return;
            formRequiredItem.classList.add("_form-success");
            formRequiredItem.parentElement.classList.add("_form-success");
            const messageSuccess = document.createElement("div");
            messageSuccess.className = "form__success";
            messageSuccess.textContent = "You have successfully subscribed!";
            setTimeout((() => {
                messageSuccess.classList.add("_show");
            }), 0);
            formRequiredItem.parentElement.append(messageSuccess);
            setTimeout((() => {
                messageSuccess.classList.remove("_show");
            }), 2e3);
            setTimeout((() => {
    ...
    function initSliders() {
        if (document.querySelector(".services__slider")) {
            const mobileMedia = window.matchMedia("(any-hover: hover)");
            mobileMedia.addEventListener("change", (e => isPc = e.matches));
            let isPc = mobileMedia.matches;
            const servicesSlider = new swiper_core_Swiper(".services__slider", {
            ...
            const servicesLinks = document.querySelectorAll(".services__slider .item-service__link");
            if (servicesLinks.length) servicesLinks.forEach(((link, index) => {
                link.addEventListener("focusin", (() => {
                    servicesSlider.slideTo(index, 600);
                }));
            }));
        }
        if (document.querySelector(".instructors__slider")) {
            const sliderInstructors = new swiper_core_Swiper(".instructors__slider", {
                modules: [ Navigation, Keyboard ],
                slidesPerView: 1,
                spaceBetween: 30,
                keyboard: true,
                ...
                initialSlide: 0,
                navigation: {
                    prevEl: ".instructors__slider .swiper-button-prev",
                    nextEl: ".instructors__slider .swiper-button-next"
                },
                breakpoints: {
                    479.98: {
                        slidesPerView: 2,
            ...
            const sliderCards = document.querySelectorAll(".instructors__slider .card-instructor__img");
            if (sliderCards.length) sliderCards.forEach(((card, index) => {
                card.addEventListener("focusin", (() => {
                    sliderInstructors.slideTo(index, 600);
                }));
            }));
        }
    ...
    parallax_Parallax.Each = class {
        ...
        animationFrame() {
            ...
            if (positionParent.top < 400 && positionParent.bottom > -400) switch (centerPoint) {
              case "top":
            ...
            this.elements.forEach((el => {
                const parameters = {
                    ...
                    direction: el.dataset.direction ? parseInt(el.dataset.direction) : "-1",
    ...
    window.addEventListener("load", (() => {
        const hash = getHash();
        if (hash && document.querySelector(`#${hash}`)) gotoBlock(`#${hash}`, true, 500, 20);
    }));
    class DynamicAdapt {
        constructor(type) {
            this.type = type;
